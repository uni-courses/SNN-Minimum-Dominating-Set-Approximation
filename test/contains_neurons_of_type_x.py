...
def get_n_neurons(
    n, neurons, neuron_dict, neuron_identifier, sample_neuron, in_simulation=False
):
...
def has_expected_custom_neuron_properties(neuron, sample_neuron, verbose=False):
    """Verifies the bias, du, dv, vth of the neuron, which are set manually,
    are as expected.."""
    if neuron.du.get() == sample_neuron.du:  # Custom value.
        ...
        if neuron.dv.get() == sample_neuron.dv:  # Default initial value.
            ...
            print(f"neuron.dv.get()={neuron.dv.get()}, whereas dv={sample_neuron.dv}")
