...
def get_dir_filelist_based_on_extension(dir_relative_to_root, extension):
    ...
    :param dir_relative_to_root: A relative directory as seen from the root dir
    of this project.
...
def dir_relative_to_root_exists(dir_relative_to_root):
    ...
    if not os.path.exists(dir_relative_to_root):
        ...
        raise Exception(
            "Directory relative to root: {dir_relative_to_root} did not exist"
            + ", nor did it exist."
        )
...
def get_all_files_in_dir_and_child_dirs(extension, path, excluded_files=None):
    ...
    function. The file extension of the file that is sought in the appendix
