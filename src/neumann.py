...
def compute_mtds(input_graph, m=0):
    # As currently no decision has been reached as to what kind of object input_graph actually is,
    # this code will accept both node and edges lists as well as nx.graphs
    ...
    if isinstance(input_graph, tuple):
        graph = nx.Graph()
        graph.add_nodes_from(input_graph[0], marks=0, random_number=0, weight=0)
    ...
    for node in graph.nodes:
        ...
        # 1.d Each vertex v_i sends w_i to each of its neighbours. (Not happening due to a centralised version)
